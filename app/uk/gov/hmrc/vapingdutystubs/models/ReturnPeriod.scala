...
import uk.gov.hmrc.vapingdutystubs.utils.DateTimeHelper.instantToLocalDate

import java.time.{Clock, Instant, LocalDate, YearMonth}
...
case class ReturnPeriod(period: YearMonth) {
  ...
  def withTodaysDayIfThisMonth(clock: Clock): LocalDate = {
    val today     = instantToLocalDate(Instant.now(clock))
    val thisMonth = YearMonth.from(today)

    if (period == thisMonth) {
      period.atDay(instantToLocalDate(Instant.now(clock)).getDayOfMonth)
    } else {
      period.atDay(1)
    }
  }
